...
@app.route("/crypto/api/overview")
def crypto_api_overview():
    try:
        ...
        #
        # Quasi-hardcoded defaults for the wallet overview.
        #
        # These are deliberately returned by the API so the renderer
        # does not have to infer its current mode from which Three.js
        # objects happen to exist.
        #
        display_type = "wallet-minigraph"
        turn_display = True

        wallet_data = wallet.make_data()
        assets = []

        for item in wallet_data.get("assets", []):
            asset = str(
                item.get("asset", "")
            ).strip().upper()
            ...
            try:
                ...
                overview_item["error"] = str(exc)

            #
            # TURN is part of the default wallet-minigraph display.
            #
            # Keep the complete make_report() result, especially
            # report["display"].  That display payload contains both
            # the rolling-backtest graphics and the future prediction
            # bars generated by turndetect.
            #
            if turn_display:
                try:
                    overview_item["turn"] = (
                    ...
                    #
                    # One failed TURN analysis must not prevent the
                    # other wallet assets from being displayed.
                    #
                    overview_item["turn_error"] = (
                        str(exc)
                    )
